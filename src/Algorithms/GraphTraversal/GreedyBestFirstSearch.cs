...
namespace Algorithms.GraphTraversal
{
    public class GreedyBestFirstSearch
    {
        ...
        public static IEnumerable<T> Explore<T>(T start, Func<T, IEnumerable<T>> getNeighbours, Func<T, int> heuristic, Func<T, bool> isEnd = null)
        {
            return gbfs(start, getNeighbours,heuristic, false, isEnd);
        }

        private static IEnumerable<T> gbfs<T>(T start, Func<T, IEnumerable<T>> getNeighbours, Func<T, int> heuristic, bool pathOnly,
            Func<T, bool> isEnd = null)
        {
            ...
            while (toVisit.Any())
            {
                ...
                {
                    toVisit.Enqueue(heuristic(neighbour), neighbour); // <= add move cost here !
        ...
        public static IEnumerable<T> FindPath<T>(T start, Func<T, IEnumerable<T>> getNeighbours, Func<T, int> heuristic, Func<T, bool> isEnd = null)
        {
            return gbfs(start, getNeighbours,heuristic, true, isEnd);
        }
    }
}
